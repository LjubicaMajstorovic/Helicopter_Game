...
public class HelloApplication extends Application {
    ...
    public void start(Stage stage) {
        ...
        Elis elis = new Elis(16.875, 39.37500000000001);
        Helicopter helicopter = new Helicopter(16.875, 39.37500000000001, elis);
        ...
        speedMeter.getTransforms().addAll(new Transform[]{new Translate(365.625, 0.0)});
        FuelMeter fuelMeter = new FuelMeter(75.0, helicopter);
        fuelMeter.getTransforms().addAll(new Transform[]{new Translate(-332.4375, -332.4375)});
        root.getChildren().addAll(helipad, speedMeter, fuelMeter, helicopter);
        root.getChildren().addAll(packages);
        root.getTransforms().addAll(new Transform[]{new Translate(375.0, 375.0)});
        ...
        scene.addEventHandler(KeyEvent.KEY_PRESSED, (event) -> {
            if (event.getCode().equals(KeyCode.UP)) {
                ...
                elis.setParked(true);
                helicopter.setParked(true);



                /*ScaleTransition  scaleTransition = new ScaleTransition(Duration.seconds(1.0), helicopterGroup);
                scaleTransition.setInterpolator(Interpolator.LINEAR);
                double centerX = helicopter.getChildren().stream()
                        .map(Node::getTranslateX)
                        .reduce(0.0, Double::sum) / helicopter.getChildren().size();
                double centerY = helicopter.getChildren().stream()
                ...
                scaleTransition.setFromX(oldScale);
                scaleTransition.setFromY(oldScale);
                scaleTransition.play();


                scaleTransition.setOnFinished(event1 -> {

                    helicopterGroup.setScaleX(scale);
                    helicopterGroup.setScaleY(scale);

                    helicopterGroup.getChildren().forEach(child -> {
                        double deltaX = child.getTranslateX() - centerX;
                        double deltaY = child.getTranslateY() - centerY;
                        double distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
                        double newDistance = distance * scale / oldScale;
                        double newDeltaX = deltaX * scale / oldScale;
                        double newDeltaY = deltaY * scale / oldScale;
                        child.setTranslateX(centerX + newDeltaX);
                        child.setTranslateY(centerY + newDeltaY);
                    });
        ...
        MyTimer.IUpdatable helicopterWrapper = (ds) -> {
            elis.update(ds, 0.995, 0.0, 750.0, 0.0, 750.0);
        ...
        MyTimer myTimer = new MyTimer(new MyTimer.IUpdatable[]{helicopterWrapper});
        myTimer.start();
